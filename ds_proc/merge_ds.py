...
def mean_string(vals):
    ...
    vals_split = [int(x) for x in vals_split]
    # print(vals_split)
    return int(np.mean(vals_split))

ds1 = pd.read_csv('../data/rb_prof_Naef/AA_depr/CTRL_1_RIBO.tsv', sep=' ')
ds1.columns = ["gene", "transcript", "position_A_site", "count"]
# print(ds)

ds2 = pd.read_csv('../data/rb_prof_Naef/AA_depr/CTRL_2_RIBO.tsv', sep=' ')
ds2.columns = ["gene", "transcript", "position_A_site", "count"]
# print(ds)

ds3 = pd.read_csv('../data/rb_prof_Naef/AA_depr/CTRL_3_RIBO.tsv', sep=' ')
...
merge_df.to_csv('../data/rb_prof_Naef/processed_data/merge_CTRL.csv')