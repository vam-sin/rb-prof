...
class RBDataset_NoBS(Dataset):
    ...
    def __getitem__(self, index):
        ...
        condition_value = [conds_dict[condition]]
        ...
        condition_value = torch.tensor(condition_value, dtype=torch.long)
        ...
        return X_ft, conds_fin, y, condition, Nnt_present, condition_value
...
class BiLSTMModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers, bidirectional, dropout, bs):
        ...
        self.condition_embedding = nn.Embedding(6, input_dim-22)

        # embedding layer 
        self.embedding = nn.Embedding(84, 256)

        # lstm model
        self.lstm = nn.LSTM(input_dim-22, hidden_dim, num_layers = n_layers, bidirectional = bidirectional, dropout = dropout, batch_first=True)
    ...
    def forward(self, src, condition_value):
        # embedding
        # src = self.embedding(src)
        # src = torch.cat((src, conds_fin), dim=2)

        # condition embedding
        condition_embedding = self.condition_embedding(condition_value)

        # remove last 22 conds fin values 
        # get depr vec which is last 1 value
        depr_vec = src[:, :, -1]
        src = src[:, :, :-22]
        # print(src.shape, depr_vec.shape, condition_embedding.shape)

        src = torch.squeeze(src, 0)
        depr_vec = torch.squeeze(depr_vec, 0)
        condition_embedding = torch.squeeze(condition_embedding, 0)
        condition_embedding = torch.unsqueeze(condition_embedding, 0)

        # iterate over src and if depr vec is 1 then concat condition embedding
        for i in range(len(src)):
            if depr_vec[i] == 1:
                # add condition embedding to src
                src[i] = src[i] + condition_embedding
...
def train(model: nn.Module, train_dataloder, bs, device, criterion, mult_factor, loss_mult_factor, optimizer, logger, epoch_num, total_epochs) -> None:
    ...
    for i, data in enumerate(train_dataloder):
        optimizer.zero_grad()
        inputs, conds_fin, labels, condition, Nnt_present, condition_value = data
        if Nnt_present == False:
            ...
            condition_value = condition_value.long().to(device)
