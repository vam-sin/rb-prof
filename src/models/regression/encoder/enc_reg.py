...
from model_utils import TransformerModel, train, evaluate, RBDataset_NoBS
...
epochs = 200
bs = 1
saved_files_name = 'E-0-DS06-NT_CBERT-BS1-PCCLoss'
...
if __name__ == '__main__':
    ...
    model = TransformerModel(input_size, nhead, d_hid, nlayers, mult_factor, dropout).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_loss, train_pr = train(model, train_dataloader, 1, device, criterion, mult_factor, loss_mult_factor, optimizer, logger)

        wandb.log({"Train Loss": train_loss, "Train Pearson": train_pr})

        logger.info("------------- Validation -------------")
        val_loss, val_pr = evaluate(model, val_dataloader, device, mult_factor, criterion, logger)
        ...
        logger.info("------------- Testing -------------")
        test_loss, test_pr = evaluate(model, test_dataloader, device, mult_factor, criterion, logger)
        elapsed = time.time() - epoch_start_time
        logger.info('-' * 89)
        logger.info(f'| end of epoch {epoch:3d} | time: {elapsed:5.2f}s | '
